...
# Просмотр подключенных клиентов и их трафик

Откройте терминал на хостовой системе, где запущены контейнеры Docker.
Для того чтобы узнать идентификатор (ID) контейнера Docker, вы можете использовать команду
...
docker ps
```
Сначала запустите терминал внутри контейнера Docker. Замените <container_name_or_id> на имя или идентификатор вашего контейнера:

```
docker exec -it <container_name_or_id> /bin/bash
```
После входа в контейнер выполните команду wg show, чтобы просмотреть информацию о подключенных клиентах WireGuard:

```
wg show
```
Эта команда выведет информацию о текущих подключениях, включая имена интерфейсов, IP-адреса клиентов, публичные ключи и другие сведения.
После завершения просмотра данных WireGuard, вы можете выйти из контейнера, нажав Ctrl + D или выполнить exit.
